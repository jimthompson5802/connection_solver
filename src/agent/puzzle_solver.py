...
async def generate_vocabulary(words):
    ...
    async def process_word(the_word):
        ...
        result = await chat_with_llm(prompt)
...
async def choose_embedvec_item(candidates):
    """
...
async def one_away_analyzer(
    ...
    single_topic_groups = [
        RecommendedGroup(
            words=x[0],
            connection_description=json.loads(x[1].content)["explanation"],
        )
        for x in single_topic_groups
        if json.loads(x[1].content)["response"] == "single"
    ]
    ...
    if selected_word_group:
        ...
        new_group = list(selected_word_group.words) + [json.loads(response.content)["word"]]
        one_away_group_recommendation = RecommendedGroup(
            words=new_group, connection_description=json.loads(response.content)["explanation"]
        )
